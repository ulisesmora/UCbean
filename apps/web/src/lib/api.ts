...
import { mockProductsApi, mockReservationsApi, mockOrdersApi, mockAuthApi } from './mocks';
...
export const productsApi = USE_MOCKS
  ? mockProductsApi
  : {
...
export const reservationsApi = USE_MOCKS
  ? mockReservationsApi
  : {
      pickupSlots: (date: string) =>
        api.get<SlotAvailability>(`/reservations/pickup/slots?date=${date}`),
      bookTable: (
        body: { partySize: number; scheduledAt: string; notes?: string },
        token: string,
      ) =>
...
export const ordersApi = USE_MOCKS
  ? mockOrdersApi
  : {
